...
if not proxy_vs_path.is_file():
    raise SystemExit(f'HOTFIX12_SWAPCHAIN_PROXY_VS_MISSING={proxy_vs_path}')
if not proxy_ps_path.is_file():
    raise SystemExit(f'HOTFIX12_SWAPCHAIN_PROXY_PS_MISSING={proxy_ps_path}')
proxy_vs = proxy_vs_path.read_bytes()
proxy_ps = proxy_ps_path.read_bytes()
if len(proxy_vs) < 64 or len(proxy_ps) < 64:
    raise SystemExit('HOTFIX12_SWAPCHAIN_PROXY_SHADER_TOO_SMALL')
...
print(f'HOTFIX12_PROXY_HEADER={proxy_out}')
manifest.write_text(
    '\n'.join([
        ...
        f'HOTFIX12_SWAPCHAIN_PROXY_VS=swap_chain_proxy_vertex_shader_dx12.cso size={len(proxy_vs)} sha256={hashlib.sha256(proxy_vs).hexdigest()}',
        f'HOTFIX12_SWAPCHAIN_PROXY_PS=swap_chain_proxy_pixel_shader_dx12.cso size={len(proxy_ps)} sha256={hashlib.sha256(proxy_ps).hexdigest()}',
...
print(f'HOTFIX12_SWAPCHAIN_PROXY_VS_BYTES={len(proxy_vs)}')
print(f'HOTFIX12_SWAPCHAIN_PROXY_PS_BYTES={len(proxy_ps)}')
print('V24HDR12_SEPARATE_SWAPCHAIN_PROXY_EMBED=PASS')
