...
required = {
    ...
    'source/dxgi/dxgi.cpp': [
        ...
        'SWAPCHAIN_CREATE_STAGE_R10_SDR',
        'SWAPCHAIN_CREATE_STAGE_COLORSPACE_FAIL_ROLLBACK_R8',
        'SWAPCHAIN_CREATE_R10_FAIL_ROLLBACK_R8',
        'v24hdr_apply_sdr_stage_hotfix8',
        'v24hdr_should_attempt_hdr()',
    ],
    'source/dxgi/dxgi_swapchain.cpp': [
        'v24hdr_dxgi_gate_hotfix5.hpp',
        'RESIZE_STAGE_R10_SDR',
        'RESIZE_STAGE_R10_READY',
        'RESIZE_R10_FAIL_ROLLBACK_R8',
        'RESIZE1_STAGE_R10_SDR',
        'RESIZE1_STAGE_R10_READY',
        'RESIZE1_R10_FAIL_ROLLBACK_R8',
        'PRESENT_ARM_HDR10_SUCCESS',
        'PRESENT_ARM_HDR10_FAILED_STAY_R10_SDR',
        'PRESENT1_ARM_HDR10_SUCCESS',
        'PRESENT1_ARM_HDR10_FAILED_STAY_R10_SDR',
        'v24hdr_swapchain_is_r10_hotfix8',
    ],
    'source/dxgi/v24hdr_dxgi_gate_hotfix5.hpp': [
        ...
        'R10_SDR_STAGE_SET phase=%s',
        'R10_SDR_STAGE_DEFAULT phase=%s',
        'COLORSPACE_SET kind=%s phase=%s',
...
# HOTFIX8 deliberately stages the native D3D12 swapchain in R10 before shader readiness,
# but keeps the SDR G22/P709 colorspace. The prohibited state is now HDR10/ST2084
# before v24hdr_should_attempt_hdr() is true.
dxgi = Path('source/dxgi/dxgi.cpp').read_text(encoding='utf-8')
swap = Path('source/dxgi/dxgi_swapchain.cpp').read_text(encoding='utf-8')
gate = Path('source/dxgi/v24hdr_dxgi_gate_hotfix5.hpp').read_text(encoding='utf-8')
if 'SWAPCHAIN_CREATE_STAGE_R10_SDR' not in dxgi:
    fail('HOTFIX8_R10_STAGE_CREATE=FAIL')
if swap.count('STAGE_R10_SDR') < 2:
    fail('HOTFIX8_R10_STAGE_RESIZE=FAIL')
if 'DXGI_COLOR_SPACE_RGB_FULL_G22_NONE_P709' not in gate:
    fail('HOTFIX8_SDR_STAGE_COLORSPACE=FAIL')
if swap.count('v24hdr_should_attempt_hdr() && !v24hdr_hdr_active()') < 2:
    fail('HOTFIX8_PRESENT_ARM_GATE=FAIL')
if swap.count('PRESENT_ARM_HDR10_SUCCESS') < 1 or swap.count('PRESENT1_ARM_HDR10_SUCCESS') < 1:
    fail('HOTFIX8_PRESENT_ACTIVATION_PATH=FAIL')
if 'v24hdr_apply_hdr10_hotfix5(_orig, "present_arm")' not in swap or 'v24hdr_apply_hdr10_hotfix5(_orig, "present1_arm")' not in swap:
    fail('HOTFIX8_PRESENT_HDR10_CALL=FAIL')
...
print('V24HDR7_VERIFY_EXACT_V31_PINNED_MAP=PASS')
print('V24HDR8_VERIFY_R10_SDR_STAGE=PASS')
print('V24HDR8_VERIFY_PRESENT_ARM_HDR10=PASS')
print('V24HDR8_VERIFY_NO_MANUAL_RESIZE_REQUIRED=PASS')
...
print('V24HDR8_VERIFY_ZERO_SIDECAR_STAYS_SDR_COLORSPACE=PASS')
