# HOTFIX15 startup HDR: source verification still preserves HOTFIX14 draw-time parity markers.
...
required = {
    ...
    'source/d3d12/v24hdr_sidecar_hotfix5.hpp': [
        ...
        'v24hdr_hotfix13_get_clone_state',
    ...
    'source/dxgi/dxgi_swapchain.cpp': [
        ...
        'KAIOZEN GOLD15 PORTABLE: CLONE_STATE_RECOVERY',
...
def validate_sidecar_header(h):
    ...
    if track < 0 or physical < 0 or not (track < physical):
        fail('HOTFIX15_SET_PSO_LOGICAL_TRACKING_INVARIANT=FAIL')
    ...
    if clear_native < 0 or clear_track < 0 or not (clear_native < clear_track):
        fail('HOTFIX15_CLEARSTATE_NATIVE_FIRST_INVARIANT=FAIL')
    ...
    for marker, native_call in [
        ...
        if replacement < 0 or native < 0 or not (replacement < native):
            fail('HOTFIX15_DRAW_TIME_REPLACEMENT_ORDER_INVARIANT=FAIL=' + marker)
...
startup_gate = function_definition(h, 'bool v24hdr_should_attempt_hdr()')
if 'return v24hdr_enabled();' not in startup_gate or 'g_v24hdr_hdr_ready.load' in startup_gate:
    fail('HOTFIX15_STARTUP_HDR_GATE=FAIL')
...
# HOTFIX15 intentionally allows early R10/HDR10, but must not reintroduce the old staged-R10-without-final-proxy path.
...
# GOLD15 portability HOTFIX1: the final proxy must use the clone's tracked state
# rather than requiring GUI-dependent PRESENT state before every frame.
final_proxy_fn = function_definition(swap, 'static void v24hdr_hotfix13_final_proxy_draw(')
for marker in [
    'v24hdr_hotfix13_get_clone_state(back, &clone_state)',
    ...
    if marker not in final_proxy_fn:
        fail('GOLD15_PORTABLE_STATE_RECOVERY_INVARIANT=FAIL missing=' + marker)
if 'v24hdr_hotfix13_clone_is_present(back)' in final_proxy_fn:
    fail('GOLD15_PORTABLE_STALE_PRESENT_GATE=FAIL')
if 'CLONE_NOT_PRESENT' in final_proxy_fn:
    fail('GOLD15_PORTABLE_STALE_CLONE_NOT_PRESENT_REASON=FAIL')
if 'back_barrier.Transition.StateBefore = D3D12_RESOURCE_STATE_PRESENT' in final_proxy_fn:
    fail('GOLD15_PORTABLE_STALE_BACKBUFFER_PRESENT_ASSUMPTION=FAIL')
# HOTFIX2: raw native final draws do not update ReShade's immediate-list
# _has_commands bookkeeping. Require the wrapper draw so the following flush
# actually closes and submits the list even when no splash/effect drew first.
for marker in [
    'KAIOZEN GOLD15 PORTABLE: IMMEDIATE_SUBMIT_ARMED',
    'api_cmd->draw(3, 1, 0, 0)',
]:
    if marker not in final_proxy_fn:
        fail('GOLD15_PORTABLE_IMMEDIATE_SUBMIT_INVARIANT=FAIL missing=' + marker)
if 'cmd->DrawInstanced(3, 1, 0, 0)' in final_proxy_fn:
    fail('GOLD15_PORTABLE_RAW_FINAL_DRAW_BYPASSES_SUBMIT_FLAG=FAIL')
print('GOLD15_PORTABLE_STATE_RECOVERY_SOURCE=PASS')
print('GOLD15_PORTABLE_IMMEDIATE_SUBMIT_SOURCE=PASS')
...
print('V24HDR5_VERIFY_CRITICAL_UBERPOST_SIDECAR_DISCOVERY=PASS')
print('V24HDR15_VERIFY_STARTUP_HDR_WITH_LATE_SIDECAR_BINDING=PASS')
print('V24HDR5_VERIFY_R10_AND_COLORSPACE_ROLLBACK_R8=PASS')
print('V24HDR15_VERIFY_STARTUP_HDR_GATE=PASS')
