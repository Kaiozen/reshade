...
print('HOTFIX16_CALIBRATION_PROFILE=MACBOOK_XDR_1000_PEAK_203_PAPER_WHITE')
print('HOTFIX16_RENODX_PEAK_NITS=1000')
print('HOTFIX16_RENODX_GAME_NITS=203')
print('HOTFIX16_RENODX_UI_NITS=203')
...
if not critical.is_file():
    raise SystemExit('HOTFIX13_CRITICAL_UBERPOST_SOURCE=FAIL missing=' + str(critical))
...
if critical_text.count(intermediate) != 1:
    raise SystemExit(
        'HOTFIX13_CRITICAL_UBERPOST_INTERMEDIATE_ANCHOR=FAIL count=' +
        str(critical_text.count(intermediate))
    )
if fused in critical_text:
    raise SystemExit('HOTFIX13_FORBIDDEN_FUSED_SWAPCHAIN_PASS=FAIL')
print('HOTFIX13_CRITICAL_UBERPOST=0x06D90AA2')
print('HOTFIX13_CRITICAL_UBERPOST_INTERMEDIATE_ONLY=PASS')
print('HOTFIX13_FINAL_SWAPCHAIN_PROXY=SEPARATE')
print('HOTFIX13_FUSED_SWAPCHAIN_PASS=ABSENT')
