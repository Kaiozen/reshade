...
# HOTFIX10: The pinned ZZZ critical UberPost emits RenoDX's intermediate color,
# while the official addon normally executes a separate swap-chain proxy shader
# that calls SwapChainPass before HDR10 presentation. Our native sidecar path has
# no RenoDX runtime swap-chain proxy, so fuse the two pure-math stages in this
# one proven critical shader. This adds no descriptors/root bindings and keeps
# the pixel-shader input/output signature unchanged.
critical = game_dir / 'd3d12' / 'UberPosts' / 'uberpost_0x06D90AA2.ps_6_0.hlsl'
if not critical.is_file():
    raise SystemExit('HOTFIX10_CRITICAL_UBERPOST_SOURCE=FAIL missing=' + str(critical))
critical_text = critical.read_text(encoding='utf-8-sig')
old_critical = 'SV_Target.xyz = renodx::draw::RenderIntermediatePass(SV_Target.xyz);'
new_critical = 'SV_Target.xyz = renodx::draw::SwapChainPass(renodx::draw::RenderIntermediatePass(SV_Target.xyz));'
if critical_text.count(old_critical) != 1:
    raise SystemExit(
        'HOTFIX10_CRITICAL_UBERPOST_INTERMEDIATE_ANCHOR=FAIL count=' +
        str(critical_text.count(old_critical))
    )
if 'renodx::draw::SwapChainPass(renodx::draw::RenderIntermediatePass' in critical_text:
    raise SystemExit('HOTFIX10_CRITICAL_UBERPOST_ALREADY_FUSED=FAIL')
critical_text = critical_text.replace(old_critical, new_critical, 1)
critical.write_text(critical_text, encoding='utf-8', newline='\n')
verify_critical = critical.read_text(encoding='utf-8')
if verify_critical.count(new_critical) != 1 or old_critical in verify_critical:
    raise SystemExit('HOTFIX10_CRITICAL_UBERPOST_FUSION_VERIFY=FAIL')
print('HOTFIX10_CRITICAL_UBERPOST=0x06D90AA2')
print('HOTFIX10_CRITICAL_UBERPOST_FUSED_SWAPCHAIN_PASS=PASS')
print('HOTFIX10_INTERMEDIATE_STAGE=RenderIntermediatePass')
print('HOTFIX10_FINAL_STAGE=SwapChainPass_HDR10_PQ')
print('HOTFIX10_ADDED_SHADER_RESOURCES=ZERO')
