...
if chunk.count(old) != 1:
    raise RuntimeError(f'HOTFIX9 Present finish anchor count={chunk.count(old)}')
chunk = chunk.replace(old, new, 1)
...
if chunk.count(old) != 1:
    raise RuntimeError(f'HOTFIX9 Present1 finish anchor count={chunk.count(old)}')
chunk = chunk.replace(old, new, 1)
s = s[:start] + chunk + s[end:]
