...
# HOTFIX12 final output pass is recorded after ReShade's D3D12 presentation work and before its immediate-list flush.
on_present_old = (
...
if s.count(on_present_old) != 1:
    raise RuntimeError(f'HOTFIX12 final Present boundary count={s.count(on_present_old)}')
s = s.replace(on_present_old, on_present_new, 1)
