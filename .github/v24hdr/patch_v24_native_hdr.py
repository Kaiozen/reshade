    ...
    uint32_t observed_crc)
{
    ...
    // ZZZ OSPRODWin3.1.0 exact observed identity from the HOTFIX6 Mac report.
    // This is deliberately NOT a generic size-only rule. It requires the exact
    // current-game CRC, exact byte size, and exact current DXBC signature
    // fingerprints observed on the user's build. Only then is the single
    // historical RenoDX critical UberPost candidate selected. Native D3D12 PSO
    // creation remains the final compatibility oracle: if the replacement is
    // incompatible with the current root signature / VS / RTV state, sidecar
    // creation fails and HDR never arms.
    if (matches == 0 &&
        observed_crc == 0x4584C304u &&
        bc.BytecodeLength == 12904u &&
        ...
        fp.output_crc == 0xDB99520Bu && fp.output_size == 148u)
    {
        ...
        if (v31 != nullptr && v31->size == 12904u && v24hdr_is_critical_uberpost_crc(v31->crc32))
        {
            ++g_v24hdr_adaptive_matches;
            reshade::log::message(
                reshade::log::level::info,
                "KAIOZEN V24 HDR SIDECAR HOTFIX7: V31_PINNED_MAP observed_crc=0x4584C304 replacement_crc=0x06D90AA2 bytes=12904 isig=0x0791E583/352 osig=0xDB99520B/148 compatibility=native_pso_creation_required.");
            return v31;
        }
    }
...
// HOTFIX8 two-stage presentation:
//   1) Keep the native swapchain in R10 with ordinary SDR G22/P709 color space.
//   2) Only switch that already-R10 swapchain to HDR10/ST2084 after a critical
//      current-ZZZ sidecar has been created successfully.
...
static bool v24hdr_apply_colorspace_hotfix8(
    IUnknown *swapchain,
    ...
    const char *kind)
{
    ...
    UINT support = 0;
    const HRESULT check = sc3->CheckColorSpaceSupport(color_space, &support);
    if (FAILED(check) || (support & DXGI_SWAP_CHAIN_COLOR_SPACE_SUPPORT_FLAG_PRESENT) == 0)
    {
        reshade::log::message(
            reshade::log::level::warning,
            "KAIOZEN V24 HDR SIDECAR HOTFIX8: COLORSPACE_UNSUPPORTED kind=%s phase=%s check=0x%08X support=0x%X.",
            kind, phase, static_cast<unsigned>(check), support);
        sc3->Release();
        return false;
    }

    const HRESULT set = sc3->SetColorSpace1(color_space);
    reshade::log::message(
        SUCCEEDED(set) ? reshade::log::level::info : reshade::log::level::warning,
        "KAIOZEN V24 HDR SIDECAR HOTFIX8: COLORSPACE_SET kind=%s phase=%s check=0x%08X support=0x%X set=0x%08X.",
        kind, phase, static_cast<unsigned>(check), support, static_cast<unsigned>(set));
    sc3->Release();
    return SUCCEEDED(set);
}

static bool v24hdr_apply_sdr_stage_hotfix8(IUnknown *swapchain, const char *phase)
{
    if (swapchain == nullptr)
        return false;

    const bool explicit_ok = v24hdr_apply_colorspace_hotfix8(
        swapchain,
        DXGI_COLOR_SPACE_RGB_FULL_G22_NONE_P709,
        phase,
        "SDR_G22_P709");
    reshade::log::message(
        explicit_ok ? reshade::log::level::info : reshade::log::level::warning,
        "KAIOZEN V24 HDR SIDECAR HOTFIX8: R10_SDR_STAGE_SET phase=%s success=%u.",
        phase, explicit_ok ? 1u : 0u);

    // Failing to explicitly select the ordinary SDR color space is not a reason
    // to destroy an otherwise valid R10 swapchain. HDR10/ST2084 is still never
    // selected here. Keep the R10 swapchain staged with its native default color
    // space and wait for the proven Present-time HDR arm.
    if (!explicit_ok)
        reshade::log::message(
            reshade::log::level::warning,
            "KAIOZEN V24 HDR SIDECAR HOTFIX8: R10_SDR_STAGE_DEFAULT phase=%s.",
            phase);
    return true;
}
...
{
    const bool ok = v24hdr_apply_colorspace_hotfix8(
        swapchain,
        DXGI_COLOR_SPACE_RGB_FULL_G2084_NONE_P2020,
        phase,
        "HDR10_ST2084_P2020");
    reshade::log::message(
        ok ? reshade::log::level::info : reshade::log::level::warning,
        "KAIOZEN V24 HDR SIDECAR HOTFIX5: HDR10_SET phase=%s success=%u.",
        phase, ok ? 1u : 0u);
    if (!ok)
        reshade::log::message(
            reshade::log::level::warning,
            "KAIOZEN V24 HDR SIDECAR HOTFIX5: HDR10_UNSUPPORTED phase=%s.",
            phase);
    return ok;
}

static bool v24hdr_swapchain_is_r10_hotfix8(IUnknown *swapchain)
{
    if (swapchain == nullptr)
        return false;
    ...
    const HRESULT hr = sc1->GetDesc1(&desc);
    sc1->Release();
    return SUCCEEDED(hr) && desc.Format == DXGI_FORMAT_R10G10B10A2_UNORM;
}
'''
Path('source/dxgi/v24hdr_dxgi_gate_hotfix5.hpp').write_text(dxgi_gate, encoding='utf-8', newline='\n')

# ======================================================================================
# DXGI creation: HOTFIX8 two-stage R10 presentation.
# Stage the native D3D12 swapchain in R10 + ordinary SDR color space from startup.
# HDR10/ST2084 itself remains disarmed until a critical current-ZZZ sidecar succeeds.
# This avoids the HOTFIX4 cyan/magenta failure (HDR colorspace with zero replacements)
# while removing HOTFIX7's impossible requirement for a later game ResizeBuffers call.
# ======================================================================================
p = Path('source/dxgi/dxgi.cpp')
s = read(p)
s = once(s, '#include "addon_manager.hpp"\n', '#include "addon_manager.hpp"\n#include "v24hdr_dxgi_gate_hotfix5.hpp"\n', 'dxgi HOTFIX8 include')
...
new = (
    ...
    '\tconst DXGI_SWAP_CHAIN_DESC1 v24hdr_requested_desc = desc;\n'
    '\tconst bool v24hdr_base_modified = modified;\n'
    '\tbool v24hdr_stage_r10 = false;\n'
    '\tif (v24hdr_candidate)\n'
    '\t{\n'
    '\t\tdesc.Format = DXGI_FORMAT_R10G10B10A2_UNORM;\n'
    '\t\tmodified = true;\n'
    '\t\tv24hdr_stage_r10 = true;\n'
    '\t\tv24hdr_set_hdr_active(false);\n'
    '\t\treshade::log::message(reshade::log::level::info, "KAIOZEN V24 HDR SIDECAR HOTFIX8: SWAPCHAIN_CREATE_STAGE_R10_SDR.");\n'
    '\t}\n'
)
...
new = (
    ...
    '\tg_in_dxgi_runtime = false;\n'
    '\tif (v24hdr_stage_r10 && SUCCEEDED(hr) && ppSwapChain != nullptr && *ppSwapChain != nullptr)\n'
    '\t{\n'
    '\t\tif (!v24hdr_apply_sdr_stage_hotfix8(*ppSwapChain, "create_stage"))\n'
    '\t\t{\n'
    '\t\t\t(*ppSwapChain)->Release();\n'
    '\t\t\t*ppSwapChain = nullptr;\n'
    '\t\t\tdesc = v24hdr_requested_desc;\n'
    '\t\t\tmodified = v24hdr_base_modified;\n'
    '\t\t\treshade::log::message(reshade::log::level::warning, "KAIOZEN V24 HDR SIDECAR HOTFIX8: SWAPCHAIN_CREATE_STAGE_COLORSPACE_FAIL_ROLLBACK_R8.");\n'
    '\t\t\tg_in_dxgi_runtime = true;\n'
    '\t\t\thr = trampoline(pFactory, pDevice, hWnd, &desc, fullscreen_desc.Windowed ? nullptr : &fullscreen_desc, pRestrictToOutput, ppSwapChain);\n'
    '\t\t\tg_in_dxgi_runtime = false;\n'
    '\t\t\tv24hdr_set_hdr_active(false);\n'
    '\t\t\tv24hdr_stage_r10 = false;\n'
    '\t\t}\n'
    '\t\telse if (v24hdr_should_attempt_hdr())\n'
    '\t\t{\n'
    '\t\t\tif (v24hdr_apply_hdr10_hotfix5(*ppSwapChain, "create_ready"))\n'
    '\t\t\t\tv24hdr_set_hdr_active(true);\n'
    '\t\t}\n'
    '\t}\n'
    '\telse if (v24hdr_stage_r10 && FAILED(hr))\n'
    '\t{\n'
    '\t\tdesc = v24hdr_requested_desc;\n'
    '\t\tmodified = v24hdr_base_modified;\n'
    '\t\treshade::log::message(reshade::log::level::warning, "KAIOZEN V24 HDR SIDECAR HOTFIX8: SWAPCHAIN_CREATE_R10_FAIL_ROLLBACK_R8 first_hr=0x%08X.", static_cast<unsigned>(hr));\n'
    '\t\tg_in_dxgi_runtime = true;\n'
    '\t\thr = trampoline(pFactory, pDevice, hWnd, &desc, fullscreen_desc.Windowed ? nullptr : &fullscreen_desc, pRestrictToOutput, ppSwapChain);\n'
    '\t\tg_in_dxgi_runtime = false;\n'
    '\t\tv24hdr_set_hdr_active(false);\n'
...
# ======================================================================================
# DXGI swapchain proxy:
# - Every game-requested R8 resize remains native R10 while the sentinel is active.
# - Before HDR_READY, R10 stays in G22/P709 SDR.
# - After HDR_READY, the first successful Present/Present1 switches that existing
#   R10 swapchain to HDR10/ST2084. No game ResizeBuffers is required.
# - A post-arm game resize can also activate/reapply HDR10.
# ======================================================================================
p = Path('source/dxgi/dxgi_swapchain.cpp')
s = read(p)
s = once(s, '#include "runtime_manager.hpp"\n', '#include "runtime_manager.hpp"\n#include "v24hdr_dxgi_gate_hotfix5.hpp"\n', 'swap HOTFIX8 include')

# Present: activate HDR only AFTER a successful SDR-staged present. That guarantees
# the next frame is both HDR-colorspace and eligible for sidecar PSO substitution.
...
if start < 0 or end < 0:
    raise RuntimeError('Present boundaries missing')
chunk = s[start:end]
old = (
...
new = (
    ...
    '\t\tv24hdr_should_attempt_hdr() && !v24hdr_hdr_active())\n'
    '\t{\n'
    '\t\tif (!v24hdr_swapchain_is_r10_hotfix8(_orig))\n'
    '\t\t{\n'
    '\t\t\treshade::log::message(reshade::log::level::warning, "KAIOZEN V24 HDR SIDECAR HOTFIX8: PRESENT_ARM_NOT_R10.");\n'
    '\t\t}\n'
    '\t\telse if (v24hdr_apply_hdr10_hotfix5(_orig, "present_arm"))\n'
    '\t\t{\n'
    '\t\t\tv24hdr_set_hdr_active(true);\n'
    '\t\t\treshade::log::message(reshade::log::level::info, "KAIOZEN V24 HDR SIDECAR HOTFIX8: PRESENT_ARM_HDR10_SUCCESS.");\n'
    '\t\t}\n'
    '\t\telse\n'
    '\t\t{\n'
    '\t\t\treshade::log::message(reshade::log::level::warning, "KAIOZEN V24 HDR SIDECAR HOTFIX8: PRESENT_ARM_HDR10_FAILED_STAY_R10_SDR.");\n'
    '\t\t}\n'
    '\t}\n'
    '\n'
    '\ton_finish_present(hr);\n'
...
if chunk.count(old) != 1:
    raise RuntimeError(f'Present activation anchor count={chunk.count(old)}')
chunk = chunk.replace(old, new, 1)
...
if start < 0 or end < 0:
    raise RuntimeError('Present1 boundaries missing')
chunk = s[start:end]
old = (
...
new = (
    ...
    '\t\tif (!v24hdr_swapchain_is_r10_hotfix8(_orig))\n'
    '\t\t{\n'
    '\t\t\treshade::log::message(reshade::log::level::warning, "KAIOZEN V24 HDR SIDECAR HOTFIX8: PRESENT1_ARM_NOT_R10.");\n'
    '\t\t}\n'
    '\t\telse if (v24hdr_apply_hdr10_hotfix5(_orig, "present1_arm"))\n'
    '\t\t{\n'
    '\t\t\tv24hdr_set_hdr_active(true);\n'
    '\t\t\treshade::log::message(reshade::log::level::info, "KAIOZEN V24 HDR SIDECAR HOTFIX8: PRESENT1_ARM_HDR10_SUCCESS.");\n'
    '\t\t}\n'
    '\t\telse\n'
    '\t\t{\n'
    '\t\t\treshade::log::message(reshade::log::level::warning, "KAIOZEN V24 HDR SIDECAR HOTFIX8: PRESENT1_ARM_HDR10_FAILED_STAY_R10_SDR.");\n'
    '\t\t}\n'
    '\t}\n'
    '\n'
    '\ton_finish_present(hr);\n'
...
if chunk.count(old) != 1:
    raise RuntimeError(f'Present1 activation anchor count={chunk.count(old)}')
chunk = chunk.replace(old, new, 1)
s = s[:start] + chunk + s[end:]

# ResizeBuffers: keep native format R10 even before readiness. Colorspace remains SDR
# until readiness. This prevents Unity's startup R8 resize calls from undoing staging.
...
new = (
    ...
    '\tconst bool v24hdr_stage_r10 = _direct3d_version == reshade::api::device_api::d3d12 && NewFormat == DXGI_FORMAT_R8G8B8A8_UNORM;\n'
    '\tif (v24hdr_stage_r10)\n'
    '\t{\n'
    ...
    '#endif\n'
    '\t\treshade::log::message(reshade::log::level::info, v24hdr_should_attempt_hdr() ?\n'
    '\t\t\t"KAIOZEN V24 HDR SIDECAR HOTFIX8: RESIZE_STAGE_R10_READY." :\n'
    '\t\t\t"KAIOZEN V24 HDR SIDECAR HOTFIX8: RESIZE_STAGE_R10_SDR.");\n'
    '\t}\n'
    '\tg_in_dxgi_runtime = true;\n'
    '\tHRESULT hr = _orig->ResizeBuffers(BufferCount, Width, Height, NewFormat, SwapChainFlags);\n'
    '\tg_in_dxgi_runtime = was_in_dxgi_runtime;\n'
    '\tif (v24hdr_stage_r10 && SUCCEEDED(hr))\n'
    '\t{\n'
    '\t\tconst bool color_ok = v24hdr_should_attempt_hdr() ?\n'
    '\t\t\tv24hdr_apply_hdr10_hotfix5(_orig, "resize_ready") :\n'
    '\t\t\tv24hdr_apply_sdr_stage_hotfix8(_orig, "resize_stage");\n'
    '\t\tif (color_ok)\n'
    '\t\t{\n'
    '\t\t\tif (v24hdr_should_attempt_hdr()) v24hdr_set_hdr_active(true);\n'
    '\t\t\telse v24hdr_set_hdr_active(false);\n'
    '\t\t}\n'
    '\t\telse\n'
    '\t\t{\n'
    '\t\t\treshade::log::message(reshade::log::level::warning, "KAIOZEN V24 HDR SIDECAR HOTFIX8: RESIZE_COLORSPACE_FAIL_ROLLBACK_R8.");\n'
    '\t\t\tg_in_dxgi_runtime = true;\n'
    ...
    '\telse if (v24hdr_stage_r10 && FAILED(hr))\n'
    '\t{\n'
    '\t\treshade::log::message(reshade::log::level::warning, "KAIOZEN V24 HDR SIDECAR HOTFIX8: RESIZE_R10_FAIL_ROLLBACK_R8 first_hr=0x%08X.", static_cast<unsigned>(hr));\n'
    '\t\tg_in_dxgi_runtime = true;\n'
    '\t\thr = _orig->ResizeBuffers(BufferCount, Width, Height, v24hdr_requested_format, SwapChainFlags);\n'
    '\t\tg_in_dxgi_runtime = was_in_dxgi_runtime;\n'
    '#if RESHADE_ADDON\n'
...
new = (
    ...
    '#endif\n'
    '\t\treshade::log::message(reshade::log::level::info, v24hdr_should_attempt_hdr() ?\n'
    '\t\t\t"KAIOZEN V24 HDR SIDECAR HOTFIX8: RESIZE1_STAGE_R10_READY." :\n'
    '\t\t\t"KAIOZEN V24 HDR SIDECAR HOTFIX8: RESIZE1_STAGE_R10_SDR.");\n'
    '\t}\n'
    '\tg_in_dxgi_runtime = true;\n'
    ...
    '\t{\n'
    '\t\tconst bool color_ok = v24hdr_should_attempt_hdr() ?\n'
    '\t\t\tv24hdr_apply_hdr10_hotfix5(_orig, "resize1_ready") :\n'
    '\t\t\tv24hdr_apply_sdr_stage_hotfix8(_orig, "resize1_stage");\n'
    '\t\tif (color_ok)\n'
    '\t\t{\n'
    '\t\t\tif (v24hdr_should_attempt_hdr()) v24hdr_set_hdr_active(true);\n'
    '\t\t\telse v24hdr_set_hdr_active(false);\n'
    ...
    '\t\telse\n'
    '\t\t{\n'
    '\t\t\treshade::log::message(reshade::log::level::warning, "KAIOZEN V24 HDR SIDECAR HOTFIX8: RESIZE1_COLORSPACE_FAIL_ROLLBACK_R8.");\n'
    '\t\t\tg_in_dxgi_runtime = true;\n'
    ...
    '\telse if (v24hdr_stage_r10 && FAILED(hr))\n'
    '\t{\n'
    '\t\treshade::log::message(reshade::log::level::warning, "KAIOZEN V24 HDR SIDECAR HOTFIX8: RESIZE1_R10_FAIL_ROLLBACK_R8 first_hr=0x%08X.", static_cast<unsigned>(hr));\n'
    '\t\tg_in_dxgi_runtime = true;\n'
    '\t\thr = static_cast<IDXGISwapChain3 *>(_orig)->ResizeBuffers1(BufferCount, Width, Height, v24hdr_requested_format, SwapChainFlags, pCreationNodeMask, present_queues.p);\n'
    '\t\tg_in_dxgi_runtime = was_in_dxgi_runtime;\n'
    '#if RESHADE_ADDON\n'
...
KAIOZEN_V24_NATIVE_HDR_R10_STAGED_PRESENT_ARM_HOTFIX8_PATCH=PASS
...
HOTFIX8_PRESENTATION=R10_SDR_STAGE_THEN_PRESENT_ARM_HDR10
SWAPCHAIN_CREATE_BEFORE_ARM=R10_SDR_G22_P709
RESIZE_BEFORE_ARM=R10_SDR_G22_P709
HDR_ACTIVATION=FIRST_SUCCESSFUL_PRESENT_AFTER_CRITICAL_SIDECAR
MANUAL_RESIZE_REQUIRED=NO
ZERO_SIDECARS_HDR10_COLORSPACE=IMPOSSIBLE_BY_DESIGN
R10_CREATE_FAILURE=ROLLBACK_R8
SDR_STAGE_COLORSPACE_FAILURE=KEEP_R10_DEFAULT_SDR
PRESENT_HDR10_COLORSPACE_FAILURE=STAY_R10_SDR
RESIZE_HDR10_COLORSPACE_FAILURE=ROLLBACK_R8
