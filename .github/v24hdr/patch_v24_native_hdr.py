...
bool v24hdr_hotfix13_get_clone_state(ID3D12Resource *original, D3D12_RESOURCE_STATES *state)
{
    if (original == nullptr)
        return false;
    std::lock_guard<std::recursive_mutex> lock(g_v24hdr_hotfix13_clone_mutex);
...
void v24hdr_hotfix13_update_clone_state(ID3D12Resource *original, D3D12_RESOURCE_STATES state)
{
    std::lock_guard<std::recursive_mutex> lock(g_v24hdr_hotfix13_clone_mutex);
    const auto it = g_v24hdr_hotfix13_clones.find(original);
    if (it != g_v24hdr_hotfix13_clones.end())
...
extern bool v24hdr_hotfix13_get_clone_state(ID3D12Resource *original, D3D12_RESOURCE_STATES *state);
extern void v24hdr_hotfix13_update_clone_state(ID3D12Resource *original, D3D12_RESOURCE_STATES state);
...
static uint64_t g_v24hdr_portable_state_recovery_count = 0;
...
static void v24hdr_hotfix13_final_proxy_draw(IDXGISwapChain *swapchain, reshade::d3d12::command_queue_impl *queue)
{
    ...
    D3D12_RESOURCE_STATES clone_state = D3D12_RESOURCE_STATE_COMMON;
    if (back == nullptr || clone == nullptr || !v24hdr_hotfix13_get_clone_state(back, &clone_state))
    { v24hdr_hotfix13_draw_fail("CLONE_NOT_REGISTERED"); return; }
    ...
    // GOLD15 PORTABILITY HOTFIX1:
    // ReShade's splash/overlay can change whether an extra swapchain transition is
    // observed at Present. The private FP16 clone is not itself a DXGI backbuffer,
    // so requiring its tracked state to already equal PRESENT made final output
    // accidentally depend on GUI activity. The tracked value is the last native
    // backbuffer transition mirrored to the clone, so both resources begin this
    // final-proxy section in that same logical state. Transition each resource
    // from the actual tracked state to the role it needs for the final draw.
    v24hdr_hotfix13_set_clone_bypass(true);
    D3D12_RESOURCE_BARRIER enter_barriers[2] = {};
    UINT enter_count = 0;
    if (clone_state != D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE)
    {
        D3D12_RESOURCE_BARRIER &clone_barrier = enter_barriers[enter_count++];
        clone_barrier.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
        clone_barrier.Transition.pResource = clone;
        clone_barrier.Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
        clone_barrier.Transition.StateBefore = clone_state;
        clone_barrier.Transition.StateAfter = D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE;
    }
    if (clone_state != D3D12_RESOURCE_STATE_RENDER_TARGET)
    {
        D3D12_RESOURCE_BARRIER &back_barrier = enter_barriers[enter_count++];
        back_barrier.Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
        back_barrier.Transition.pResource = back;
        back_barrier.Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
        back_barrier.Transition.StateBefore = clone_state;
        back_barrier.Transition.StateAfter = D3D12_RESOURCE_STATE_RENDER_TARGET;
    }
    if (enter_count != 0)
        cmd->ResourceBarrier(enter_count, enter_barriers);

    if (clone_state != D3D12_RESOURCE_STATE_PRESENT)
    {
        const uint64_t recovery = ++g_v24hdr_portable_state_recovery_count;
        if (recovery <= 32 || (recovery % 120u) == 0)
            reshade::log::message(reshade::log::level::info,
                "KAIOZEN GOLD15 PORTABLE: CLONE_STATE_RECOVERY count=%llu before=0x%08X after=PIXEL_SHADER_RESOURCE.",
                static_cast<unsigned long long>(recovery), static_cast<unsigned>(clone_state));
    }
    ...
    // GOLD15 PORTABILITY HOTFIX2: The D3D12 immediate command list only submits
    // when ReShade's wrapper has marked it as containing commands. The old raw
    // DrawInstanced call bypassed that bookkeeping, so with the splash hidden the
    // subsequent flush could see _has_commands == false and skip ExecuteCommandLists.
    // Route the same full-screen draw through the wrapper; it issues the same native
    // DrawInstanced while setting the immediate-list submission flag.
    const uint64_t submit_arm = g_v24hdr_hotfix13_proxy_draw_count + 1;
    if (submit_arm <= 32 || (submit_arm % 120u) == 0)
        reshade::log::message(reshade::log::level::info,
            "KAIOZEN GOLD15 PORTABLE: IMMEDIATE_SUBMIT_ARMED count=%llu via=api_command_list_draw.",
            static_cast<unsigned long long>(submit_arm));
    api_cmd->draw(3, 1, 0, 0);
    D3D12_RESOURCE_BARRIER exit_barriers[2] = {};
    exit_barriers[0].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
    exit_barriers[0].Transition.pResource = clone;
    exit_barriers[0].Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
    exit_barriers[0].Transition.StateBefore = D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE;
    exit_barriers[0].Transition.StateAfter = D3D12_RESOURCE_STATE_PRESENT;
    exit_barriers[1].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
    exit_barriers[1].Transition.pResource = back;
    exit_barriers[1].Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
    exit_barriers[1].Transition.StateBefore = D3D12_RESOURCE_STATE_RENDER_TARGET;
    exit_barriers[1].Transition.StateAfter = D3D12_RESOURCE_STATE_PRESENT;
    cmd->ResourceBarrier(2, exit_barriers);
    v24hdr_hotfix13_update_clone_state(back, D3D12_RESOURCE_STATE_PRESENT);
    v24hdr_hotfix13_set_clone_bypass(false);
...
HOTFIX13_NO_PRESENT_COPY=PASS\nHOTFIX13_FP16_BACKBUFFER_CLONE=PASS\nGOLD15_PORTABLE_STATE_RECOVERY=TRACKED_STATE_TO_CLONE_PSR_AND_BACK_RTV_THEN_PRESENT
