    ...
    uint32_t observed_crc)
{
        ...
        fp.output_crc == 0xDB99520Bu && fp.output_size == 148u)
    {
        ...
        // HOTFIX10C: The generated shader header already carries the compiled blob size.
        // Do not duplicate that size as a brittle runtime gate. Exact current-game identity
        // plus the pinned replacement key selects the candidate; native PSO creation remains
        // the final compatibility oracle.
        if (v31 != nullptr && v24hdr_is_critical_uberpost_crc(v31->crc32))
        {
            ++g_v24hdr_adaptive_matches;
            reshade::log::message(
                reshade::log::level::info,
                "KAIOZEN V24 HDR SIDECAR HOTFIX10C: V31_PINNED_MAP observed_crc=0x4584C304 replacement_crc=0x06D90AA2 observed_bytes=12904 replacement_bytes=%llu isig=0x0791E583/352 osig=0xDB99520B/148 size_gate=header_driven compatibility=native_pso_creation_required.",
...
HOTFIX10C_PINNED_MAP=HEADER_DRIVEN_REPLACEMENT_SIZE
HOTFIX10C_FUSED_SHADER_MANIFEST_BYTES=13928
