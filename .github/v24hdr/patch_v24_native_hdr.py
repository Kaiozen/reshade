    ...
    const char *source)
{
    ...
    if (crc == 0x06D90AA2u)
        reshade::log::message(reshade::log::level::info,
            "KAIOZEN V24 HDR OUTPUT HOTFIX12: UBERPOST_RTV count=%u format0=%d sample_count=%u.",
...
new = (
    ...
    '\t{\n'
    '\t\tif (v24hdr_hotfix12_prepare_final_proxy(*ppSwapChain, "create") && v24hdr_apply_hdr10_hotfix5(*ppSwapChain, "create"))\n'
    '\t\t{\n'
...
# HOTFIX12: RenoDX's critical UberPost remains an intermediate pass. Encode that
...
static std::recursive_mutex g_v24hdr_hotfix12_proxy_mutex;
static IDXGISwapChain *g_v24hdr_hotfix12_proxy_owner = nullptr;
static com_ptr<ID3D12RootSignature> g_v24hdr_hotfix12_proxy_root_signature;
static com_ptr<ID3D12PipelineState> g_v24hdr_hotfix12_proxy_pso;
static com_ptr<ID3D12DescriptorHeap> g_v24hdr_hotfix12_proxy_srv_heap;
static com_ptr<ID3D12DescriptorHeap> g_v24hdr_hotfix12_proxy_rtv_heap;
static com_ptr<ID3D12Resource> g_v24hdr_hotfix12_proxy_source;
static UINT g_v24hdr_hotfix12_proxy_width = 0;
static UINT g_v24hdr_hotfix12_proxy_height = 0;
static UINT g_v24hdr_hotfix12_proxy_buffers = 0;
static UINT g_v24hdr_hotfix12_proxy_rtv_stride = 0;
static DXGI_FORMAT g_v24hdr_hotfix12_proxy_format = DXGI_FORMAT_UNKNOWN;
static uint64_t g_v24hdr_hotfix12_proxy_prepare_count = 0;
static uint64_t g_v24hdr_hotfix12_proxy_draw_count = 0;
static uint64_t g_v24hdr_hotfix12_proxy_draw_fail_count = 0;

static void v24hdr_hotfix12_reset_final_proxy_locked()
{
    g_v24hdr_hotfix12_proxy_source.reset();
    g_v24hdr_hotfix12_proxy_rtv_heap.reset();
    g_v24hdr_hotfix12_proxy_srv_heap.reset();
    g_v24hdr_hotfix12_proxy_pso.reset();
    g_v24hdr_hotfix12_proxy_root_signature.reset();
    g_v24hdr_hotfix12_proxy_owner = nullptr;
    g_v24hdr_hotfix12_proxy_width = 0;
    g_v24hdr_hotfix12_proxy_height = 0;
    g_v24hdr_hotfix12_proxy_buffers = 0;
    g_v24hdr_hotfix12_proxy_rtv_stride = 0;
    g_v24hdr_hotfix12_proxy_format = DXGI_FORMAT_UNKNOWN;
}

static bool v24hdr_hotfix12_prepare_fail(
    const char *phase, HRESULT hr, ID3D12Resource *back0, ID3D12Device *device, IDXGISwapChain3 *sc3)
{
    if (back0 != nullptr) back0->Release();
    if (device != nullptr) device->Release();
    if (sc3 != nullptr) sc3->Release();
    v24hdr_hotfix12_reset_final_proxy_locked();
    reshade::log::message(reshade::log::level::warning,
        "KAIOZEN V24 HDR OUTPUT HOTFIX12: FINAL_PROXY_PREPARE_FAIL phase=%s hr=0x%08X HDR_BLOCKED=1.",
        phase, static_cast<unsigned>(hr));
    return false;
}

bool v24hdr_hotfix12_prepare_final_proxy(IDXGISwapChain *swapchain, const char *phase)
{
    std::lock_guard<std::recursive_mutex> lock(g_v24hdr_hotfix12_proxy_mutex);
    if (swapchain == nullptr)
        return false;
    ...
    if (FAILED(hr) || sc3 == nullptr)
        return v24hdr_hotfix12_prepare_fail(phase, hr, back0, device, sc3);

    DXGI_SWAP_CHAIN_DESC desc = {};
    hr = sc3->GetDesc(&desc);
    if (FAILED(hr) || desc.BufferDesc.Format != DXGI_FORMAT_R10G10B10A2_UNORM || desc.BufferCount == 0)
        return v24hdr_hotfix12_prepare_fail(phase, FAILED(hr) ? hr : E_INVALIDARG, back0, device, sc3);

    hr = sc3->GetDevice(IID_PPV_ARGS(&device));
    if (FAILED(hr) || device == nullptr)
        return v24hdr_hotfix12_prepare_fail(phase, hr, back0, device, sc3);
    hr = sc3->GetBuffer(0, IID_PPV_ARGS(&back0));
    if (FAILED(hr) || back0 == nullptr)
        return v24hdr_hotfix12_prepare_fail(phase, hr, back0, device, sc3);

    const D3D12_RESOURCE_DESC back_desc = back0->GetDesc();
    if (back_desc.Dimension != D3D12_RESOURCE_DIMENSION_TEXTURE2D ||
        back_desc.Format != DXGI_FORMAT_R10G10B10A2_UNORM || back_desc.SampleDesc.Count != 1 ||
        back_desc.Width == 0 || back_desc.Height == 0)
        return v24hdr_hotfix12_prepare_fail(phase, E_INVALIDARG, back0, device, sc3);

    if (g_v24hdr_hotfix12_proxy_owner == swapchain &&
        g_v24hdr_hotfix12_proxy_width == static_cast<UINT>(back_desc.Width) &&
        g_v24hdr_hotfix12_proxy_height == back_desc.Height &&
        g_v24hdr_hotfix12_proxy_buffers == desc.BufferCount &&
        g_v24hdr_hotfix12_proxy_format == back_desc.Format &&
        g_v24hdr_hotfix12_proxy_pso != nullptr && g_v24hdr_hotfix12_proxy_source != nullptr)
    {
        back0->Release();
        device->Release();
        sc3->Release();
    ...
    if (errors != nullptr) errors->Release();
    hr = device->CreateRootSignature(0, serialized->GetBufferPointer(), serialized->GetBufferSize(), IID_PPV_ARGS(&g_v24hdr_hotfix12_proxy_root_signature));
    serialized->Release();
    if (FAILED(hr) || g_v24hdr_hotfix12_proxy_root_signature == nullptr)
        return v24hdr_hotfix12_prepare_fail(phase, hr, back0, device, sc3);

    D3D12_GRAPHICS_PIPELINE_STATE_DESC pso = {};
    pso.pRootSignature = g_v24hdr_hotfix12_proxy_root_signature.get();
    ...
    hr = device->CreateGraphicsPipelineState(&pso, IID_PPV_ARGS(&g_v24hdr_hotfix12_proxy_pso));
    if (FAILED(hr) || g_v24hdr_hotfix12_proxy_pso == nullptr)
        return v24hdr_hotfix12_prepare_fail(phase, hr, back0, device, sc3);
    ...
    hr = device->CreateDescriptorHeap(&srv_heap_desc, IID_PPV_ARGS(&g_v24hdr_hotfix12_proxy_srv_heap));
    if (FAILED(hr))
        return v24hdr_hotfix12_prepare_fail(phase, hr, back0, device, sc3);
    ...
    hr = device->CreateDescriptorHeap(&rtv_heap_desc, IID_PPV_ARGS(&g_v24hdr_hotfix12_proxy_rtv_heap));
    if (FAILED(hr))
        return v24hdr_hotfix12_prepare_fail(phase, hr, back0, device, sc3);

    D3D12_RESOURCE_DESC source_desc = back_desc;
    source_desc.Flags = D3D12_RESOURCE_FLAG_NONE;
    D3D12_HEAP_PROPERTIES heap = {};
    heap.Type = D3D12_HEAP_TYPE_DEFAULT;
    heap.CreationNodeMask = 1;
    heap.VisibleNodeMask = 1;
    hr = device->CreateCommittedResource(&heap, D3D12_HEAP_FLAG_NONE, &source_desc, D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(&g_v24hdr_hotfix12_proxy_source));
    if (FAILED(hr) || g_v24hdr_hotfix12_proxy_source == nullptr)
        return v24hdr_hotfix12_prepare_fail(phase, hr, back0, device, sc3);
    ...
    device->CreateShaderResourceView(g_v24hdr_hotfix12_proxy_source.get(), &srv, g_v24hdr_hotfix12_proxy_srv_heap->GetCPUDescriptorHandleForHeapStart());

    g_v24hdr_hotfix12_proxy_rtv_stride = device->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
    D3D12_CPU_DESCRIPTOR_HANDLE rtv = g_v24hdr_hotfix12_proxy_rtv_heap->GetCPUDescriptorHandleForHeapStart();
    for (UINT i = 0; i < desc.BufferCount; ++i)
    {
        ...
        buffer->Release();
        rtv.ptr += g_v24hdr_hotfix12_proxy_rtv_stride;
    }

    g_v24hdr_hotfix12_proxy_owner = swapchain;
    g_v24hdr_hotfix12_proxy_width = static_cast<UINT>(back_desc.Width);
    g_v24hdr_hotfix12_proxy_height = back_desc.Height;
    g_v24hdr_hotfix12_proxy_buffers = desc.BufferCount;
    g_v24hdr_hotfix12_proxy_format = back_desc.Format;
    ++g_v24hdr_hotfix12_proxy_prepare_count;
    reshade::log::message(reshade::log::level::info,
        "KAIOZEN V24 HDR OUTPUT HOTFIX12: FINAL_PROXY_PREPARE=PASS phase=%s count=%llu width=%u height=%u buffers=%u vs_bytes=%llu ps_bytes=%llu.",
        phase, static_cast<unsigned long long>(g_v24hdr_hotfix12_proxy_prepare_count), g_v24hdr_hotfix12_proxy_width, g_v24hdr_hotfix12_proxy_height,
        g_v24hdr_hotfix12_proxy_buffers, static_cast<unsigned long long>(g_v24hdr_hotfix12_proxy_vs_size), static_cast<unsigned long long>(g_v24hdr_hotfix12_proxy_ps_size));
...
static void v24hdr_hotfix12_draw_fail(ID3D12Resource *back, IDXGISwapChain3 *sc3, const char *reason)
{
    ++g_v24hdr_hotfix12_proxy_draw_fail_count;
    reshade::log::message(reshade::log::level::warning,
        "KAIOZEN V24 HDR OUTPUT HOTFIX12: FINAL_PROXY_DRAW_FAIL reason=%s count=%llu.",
        reason, static_cast<unsigned long long>(g_v24hdr_hotfix12_proxy_draw_fail_count));
    if (back != nullptr) back->Release();
    if (sc3 != nullptr) sc3->Release();
}

static void v24hdr_hotfix12_final_proxy_draw(IDXGISwapChain *swapchain, D3D12CommandQueue *queue)
{
    if (!v24hdr_hdr_active() || swapchain == nullptr || queue == nullptr)
        return;

    std::lock_guard<std::recursive_mutex> lock(g_v24hdr_hotfix12_proxy_mutex);
    if (g_v24hdr_hotfix12_proxy_owner != swapchain || g_v24hdr_hotfix12_proxy_pso == nullptr ||
        g_v24hdr_hotfix12_proxy_source == nullptr || g_v24hdr_hotfix12_proxy_srv_heap == nullptr ||
        g_v24hdr_hotfix12_proxy_rtv_heap == nullptr)
    {
        v24hdr_hotfix12_draw_fail(nullptr, nullptr, "NOT_PREPARED");
        return;
    }
    ...
    if (FAILED(hr) || sc3 == nullptr)
    {
        v24hdr_hotfix12_draw_fail(back, sc3, "QUERY_SWAPCHAIN3");
        return;
    }
    const UINT index = sc3->GetCurrentBackBufferIndex();
    hr = index < g_v24hdr_hotfix12_proxy_buffers ? sc3->GetBuffer(index, IID_PPV_ARGS(&back)) : E_INVALIDARG;
    if (FAILED(hr) || back == nullptr)
    {
        v24hdr_hotfix12_draw_fail(back, sc3, "GET_BACKBUFFER");
        return;
    }
    ...
    if (cmd == nullptr)
    {
        v24hdr_hotfix12_draw_fail(back, sc3, "NO_IMMEDIATE_COMMAND_LIST");
        return;
    }

    D3D12_RESOURCE_BARRIER barriers[2] = {};
    barriers[0].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
    barriers[0].Transition.pResource = back;
    barriers[0].Transition.Subresource = D3D12_RESOURCE_BARRIER_ALL_SUBRESOURCES;
    barriers[0].Transition.StateBefore = D3D12_RESOURCE_STATE_PRESENT;
    barriers[0].Transition.StateAfter = D3D12_RESOURCE_STATE_COPY_SOURCE;
    cmd->ResourceBarrier(1, barriers);
    cmd->CopyResource(g_v24hdr_hotfix12_proxy_source.get(), back);

    barriers[0].Transition.pResource = g_v24hdr_hotfix12_proxy_source.get();
    ...
    ID3D12DescriptorHeap *heaps[] = { g_v24hdr_hotfix12_proxy_srv_heap.get() };
    cmd->SetDescriptorHeaps(1, heaps);
    cmd->SetGraphicsRootSignature(g_v24hdr_hotfix12_proxy_root_signature.get());
    cmd->SetPipelineState(g_v24hdr_hotfix12_proxy_pso.get());
    cmd->SetGraphicsRootDescriptorTable(0, g_v24hdr_hotfix12_proxy_srv_heap->GetGPUDescriptorHandleForHeapStart());
    cmd->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);

    D3D12_VIEWPORT viewport = {};
    viewport.Width = static_cast<float>(g_v24hdr_hotfix12_proxy_width);
    viewport.Height = static_cast<float>(g_v24hdr_hotfix12_proxy_height);
    viewport.MinDepth = 0.0f;
    viewport.MaxDepth = 1.0f;
    D3D12_RECT scissor = { 0, 0, static_cast<LONG>(g_v24hdr_hotfix12_proxy_width), static_cast<LONG>(g_v24hdr_hotfix12_proxy_height) };
    cmd->RSSetViewports(1, &viewport);
    cmd->RSSetScissorRects(1, &scissor);
    D3D12_CPU_DESCRIPTOR_HANDLE rtv = g_v24hdr_hotfix12_proxy_rtv_heap->GetCPUDescriptorHandleForHeapStart();
    rtv.ptr += static_cast<SIZE_T>(index) * g_v24hdr_hotfix12_proxy_rtv_stride;
    ...
    ++g_v24hdr_hotfix12_proxy_draw_count;
    if (g_v24hdr_hotfix12_proxy_draw_count <= 16 || (g_v24hdr_hotfix12_proxy_draw_count % 120u) == 0)
        reshade::log::message(reshade::log::level::info,
            "KAIOZEN V24 HDR OUTPUT HOTFIX12: FINAL_PROXY_DRAW count=%llu backbuffer=%u width=%u height=%u.",
            static_cast<unsigned long long>(g_v24hdr_hotfix12_proxy_draw_count), index, g_v24hdr_hotfix12_proxy_width, g_v24hdr_hotfix12_proxy_height);
    back->Release();
    sc3->Release();
}
...
static void v24hdr_hotfix9_present_transition(DXGISwapChain *proxy, IDXGISwapChain *original, const char *phase)
{
    ...
    {
        if (v24hdr_hotfix12_prepare_final_proxy(original, "present_existing_r10") && v24hdr_apply_hdr10_hotfix5(original, "present_existing_r10"))
        {
            v24hdr_set_hdr_active(true);
            InterlockedExchange(&g_v24hdr_hotfix9_transition_state, 2);
...
# HOTFIX12A builder compatibility fix: anchor the separate final output pass to the
# unique D3D12 immediate-list flush inside DXGISwapChain::on_present(). Inherited
# V24 patches may insert statements between present_effect_runtime() and the flush,
# so adjacency is intentionally NOT required. Runtime placement remains after all
# existing D3D12 presentation work and immediately before the final flush.
# HOTFIX12_FINAL_PRESENT_ANCHOR=FUNCTION_SCOPED_D3D12_FLUSH
...
if on_present_start < 0 or on_present_end < 0:
    raise RuntimeError('HOTFIX12A on_present boundaries missing')
on_present_chunk = s[on_present_start:on_present_end]
d3d12_case = on_present_chunk.find('case reshade::api::device_api::d3d12:')
if d3d12_case < 0:
    raise RuntimeError('HOTFIX12A D3D12 on_present case missing')
d3d12_chunk = on_present_chunk[d3d12_case:]
flush_anchor = '\t\tstatic_cast<D3D12CommandQueue *>(_direct3d_command_queue)->flush_immediate_command_list();\n'
if d3d12_chunk.count(flush_anchor) != 1:
    raise RuntimeError(f'HOTFIX12A D3D12 final flush anchor count={d3d12_chunk.count(flush_anchor)}')
flush_pos = d3d12_chunk.find(flush_anchor)
present_effect_pos = d3d12_chunk.rfind('reshade::present_effect_runtime(_impl);', 0, flush_pos)
if present_effect_pos < 0:
    raise RuntimeError('HOTFIX12A D3D12 present_effect_runtime missing before final flush')
proxy_insert = (
    '\t\tif (v24hdr_hdr_active())\n'
    '\t\t\tv24hdr_hotfix12_final_proxy_draw(_orig, static_cast<D3D12CommandQueue *>(_direct3d_command_queue));\n'
)
d3d12_chunk = d3d12_chunk[:flush_pos] + proxy_insert + d3d12_chunk[flush_pos:]
...
new = (
    ...
    '\t{\n'
    '\t\tif (v24hdr_hotfix12_prepare_final_proxy(_orig, "resize") && v24hdr_apply_hdr10_hotfix5(_orig, "resize"))\n'
    '\t\t\tv24hdr_set_hdr_active(true);\n'
    '\t\telse\n'
    '\t\t{\n'
...
new = (
    ...
    '\tif (v24hdr_attempt && SUCCEEDED(hr))\n'
    '\t{\n'
    '\t\tif (v24hdr_hotfix12_prepare_final_proxy(_orig, "resize1") && v24hdr_apply_hdr10_hotfix5(_orig, "resize1"))\n'
    '\t\t\tv24hdr_set_hdr_active(true);\n'
    '\t\telse\n'
    '\t\t{\n'
...
HOTFIX12_UBERPOST_STAGE=RENDER_INTERMEDIATE_ONLY
HOTFIX12_FINAL_OUTPUT_STAGE=SEPARATE_NATIVE_SWAPCHAIN_PROXY
HOTFIX12_FINAL_PROXY_POSITION=AFTER_PRESENT_EFFECT_RUNTIME_BEFORE_D3D12_IMMEDIATE_FLUSH
HOTFIX12_FINAL_ENCODING=HDR10_PQ_BT2020
HOTFIX12_PROXY_PREPARE_REQUIRED_BEFORE_HDR_ACTIVE=YES
HOTFIX12_NO_FUSED_PQ_IN_UBERPOST=PASS
