...
static void v24hdr_install_command_list_hooks(IUnknown *object)
{
    ...
    if (fresh)
    {
        ...
        if (reset || setpso || barriers)
            ++g_v24hdr_command_list_hooks;
        reshade::log::message(reshade::log::level::info,
            "KAIOZEN V24 HDR SIDECAR HOTFIX13: COMMAND_LIST_HOOK reset=%u set_pso=%u barriers=%u vtable=%p total=%llu.",
            reset ? 1u : 0u, setpso ? 1u : 0u, barriers ? 1u : 0u, vtable,
...
static void v24hdr_install_device_hooks(ID3D12Device *device)
{
    ...
    reshade::log::message(
        reshade::log::level::info,
        "KAIOZEN V24 HDR SIDECAR HOTFIX13: ACTIVE shader_count=%llu native_identity=1 addon_runtime=0 sidecar_psos=1 arm_before_hdr=1 fp16_backbuffer_clones=1 graphics_hook=%u stream_hook=%u library_hook=%u command_list_hook=%u command_list1_hook=%u srv_hook=%u rtv_hook=%u.",
