    ...
    uint32_t &crc)
{
    ...
    if (bc.pShaderBytecode == nullptr || bc.BytecodeLength == 0)
        return false;
    crc = compute_crc32(static_cast<const uint8_t *>(bc.pShaderBytecode), bc.BytecodeLength);
    const auto *replacement = v24hdr_find_shader(crc);
    bool log_crc = false;
    {
        std::lock_guard<std::mutex> lock(g_v24hdr_sidecar_mutex);
        if (g_v24hdr_observed_crc_set.size() < 512)
            log_crc = g_v24hdr_observed_crc_set.insert(crc).second;
    }
    if (log_crc)
    {
        reshade::log::message(
            reshade::log::level::info,
            "KAIOZEN V24 HDR SIDECAR HOTFIX5: PS_CRC_OBSERVED crc=0x%08X matched=%u bytes=%llu.",
            crc, replacement != nullptr ? 1u : 0u,
