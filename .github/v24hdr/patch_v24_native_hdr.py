...
static void v24hdr_install_device_hooks(ID3D12Device *device)
{
    ...
    reshade::log::message(
        reshade::log::level::info,
        "KAIOZEN V24 HDR POLISH HOTFIX16: ACTIVE startup_hdr=1 renodx_draw_parity=1 fp16_clone=1 proxy_defer_retry=1 calibration_peak_nits=1000 calibration_game_nits=203 calibration_ui_nits=203 shader_coverage_telemetry=1.");
}
'''
...
static uint64_t g_v24hdr_hotfix16_proxy_draw_defer_count = 0;
...
static void v24hdr_hotfix16_draw_defer(const char *reason)
{
    const uint64_t count = ++g_v24hdr_hotfix16_proxy_draw_defer_count;
    if (count <= 16 || (count % 512u) == 0)
        reshade::log::message(reshade::log::level::info,
            "KAIOZEN V24 HDR OUTPUT HOTFIX16: FINAL_PROXY_DRAW_DEFER reason=%s count=%llu retry_next_present=1.",
...
static void v24hdr_hotfix13_final_proxy_draw(IDXGISwapChain *swapchain, reshade::d3d12::command_queue_impl *queue)
{
    ...
    { v24hdr_hotfix16_draw_defer("CLONE_NOT_PRESENT"); return; }
...
HOTFIX16_PROXY_NOT_PRESENT_POLICY=DEFER_RETRY_NEXT_PRESENT
HOTFIX16_CALIBRATION_PROFILE=MACBOOK_XDR_1000_PEAK_203_PAPER_WHITE
HOTFIX16_SHADER_COVERAGE_TELEMETRY=FULL_MANIFEST_SIDECAR_DRAW_LISTS
