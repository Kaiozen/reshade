...
static void v24hdr_hotfix12_final_proxy_draw(IDXGISwapChain *swapchain, reshade::d3d12::command_queue_impl *queue)
{
...
# HOTFIX12B native-bridge compatibility fix: the inherited V24 D3DMetal bridge
...
# HOTFIX12_FINAL_PRESENT_ANCHOR=PRIVATE_D3D12_IMPL_FLUSH
...
if on_present_start < 0 or on_present_end < 0:
    raise RuntimeError('HOTFIX12B on_present boundaries missing')
on_present_chunk = s[on_present_start:on_present_end]
d3d12_case = on_present_chunk.find('case reshade::api::device_api::d3d12:')
if d3d12_case < 0:
    raise RuntimeError('HOTFIX12B D3D12 on_present case missing')
...
if d3d12_chunk.count(flush_token) != 1:
    raise RuntimeError(f'HOTFIX12B private D3D12 final flush count={d3d12_chunk.count(flush_token)}')
...
if present_effect_pos < 0:
    raise RuntimeError('HOTFIX12B D3D12 present_effect_runtime missing before private final flush')
line_start = d3d12_chunk.rfind('\n', 0, flush_pos) + 1
indent = d3d12_chunk[line_start:flush_pos]
if indent.strip() != '':
    raise RuntimeError('HOTFIX12B private D3D12 flush indentation parse failed')
proxy_insert = (
    indent + 'if (v24hdr_hdr_active())\n'
    + indent + '\tv24hdr_hotfix12_final_proxy_draw(_orig, _direct3d_command_queue_impl);\n'
)
...
if 'static_cast<D3D12CommandQueue *>(_direct3d_command_queue)' in s:
    raise RuntimeError('HOTFIX12B proxy-only D3D12 queue cast reintroduced')
