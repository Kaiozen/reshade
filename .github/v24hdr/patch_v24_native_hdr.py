    ...
    uint32_t observed_crc)
{
        ...
        fp.output_crc == 0xDB99520Bu && fp.output_size == 148u)
    {
        ...
        // HOTFIX10B: HOTFIX10 fused RenderIntermediatePass+SwapChainPass grows
        // the compiled replacement from 12904 to the audited 13928 bytes.
        if (v31 != nullptr && v31->size == 13928u && v24hdr_is_critical_uberpost_crc(v31->crc32))
        {
            ++g_v24hdr_adaptive_matches;
            reshade::log::message(
                reshade::log::level::info,
                "KAIOZEN V24 HDR SIDECAR HOTFIX7: V31_PINNED_MAP observed_crc=0x4584C304 replacement_crc=0x06D90AA2 observed_bytes=12904 replacement_bytes=13928 isig=0x0791E583/352 osig=0xDB99520B/148 compatibility=native_pso_creation_required.");
            return v31;
        }
    }
