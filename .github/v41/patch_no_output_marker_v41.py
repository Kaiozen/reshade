...
if "D3DMetal RTX exact FP32 division bridge v37:" not in text:
    raise RuntimeError("V37 must be applied before V41")
...
if text.count(anchor) != 1:
    raise RuntimeError(
        f"V41 V32 function anchor mismatch: {text.count(anchor)}"
    )
...
marker = (
    ...
    "\t\t\t\t\t\"D3DMetal RTX no-output raygen control v41: ACTIVE trace_ray=0 scene_reads=0 output_stores=0 execute_body=ret_void.\");\n"
    "\t\t\t});\n"
)
...
report.write_text(
    "\n".join([
        ...
        "V37_PIPELINE_SCAFFOLD_PRESERVED=YES",
